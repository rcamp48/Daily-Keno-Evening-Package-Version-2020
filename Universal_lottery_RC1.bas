...
REM *** Lotto Ken Evening Simulator Version 2020 RC7 Working Version With Automatic Draw Date Update
...
DIM temp(maxdraw)
DIM temp2(maxdraw)
DIM temp3(maxnumbers)
DIM temp4(maxnumbers)
DIM entry(maxnumbers)
DIM enternumbers(maxdraw, maxnumbers)
DIM entrypick(maxdraw)
DIM month$(12)
DIM numb(maxnumbers)
DIM chart(50000000, maxdraw + bonus)
DIM bonus(maxdraw + bonus)
DIM outpt(maxnumbers + bonus)
DIM outpt1(maxnumbers + bonus)
DIM outpt2(maxnumbers + bonus)
DIM maxline(maxlines)
DIM entry5(maxnumbers + bonus)
DIM entry3(maxnumbers + bonus)
DIM entry2(maxentry + bonus)
DIM m AS INTEGER
DIM y AS INTEGER
DIM day2 AS INTEGER
DIM SHARED draw2$(maxnumbers + bonus)
...
FOR zq = 1 TO qqq
    ...
    IF entry(zq) = 0 THEN
        ...
        GOTO main
    END IF
    IF entry(zq) < 1 OR entry(zq) > maxnos THEN
...
FOR x = 1 TO sets
    FOR zzzr = 1 TO maxsets
        ...
        FOR i = 1 TO maxdraw
            enterynumber = entrypick(maxdraw)
        NEXT i
        ...
        END IF
        FOR i = 1 TO maxlines
            FOR j = 1 TO maxline - 1
                IF maxdraw = maxlines THEN
                    pattern1$ = "## "
                    pattern2$ = "## "
                    PRINT USING pattern1$; maxline;
                END IF
                PRINT USING "##"; maxline
            NEXT j
        NEXT i
...
NEXT x
CLS
PRINT "You have bought"; sets * maxsets; "sets of tickets at $5.00 per set."
PRINT "This has cost you about $"; sets * price; " dollars worth of tickets."
PRINT
...
PRINT "You have bought "; sets; "sets of 7 numbers from 1 to "; maxnumbers
PRINT
PRINT
PRINT "Press any key to continue..."
DO WHILE INKEY$ = ""
...
PRINT
DO UNTIL EOF(1)
    FOR i = 1 TO maxnumbers + bonus + bonus
        IF entry(i) = entry2(j) THEN
            match = match + 1
        END IF
        IF entry(i) = entry2(j) AND match = matchnumbers THEN
            match = match + 1
        END IF
    NEXT i
        ...
        FOR i = 1 TO maxnumbers + bonus
            FOR j = 1 TO maxline - 1
                IF maxdraw = maxlines THEN
                    pattern1$ = "## "
                    pattern2$ = "## "
                    PRINT USING pattern1$; chart(i, maxnumbers);
                END IF
                PRINT USING "##"; chart(i, maxnumbers)
            NEXT j
        NEXT i
            ...
            FOR i = 1 TO maxnumbers + bonus
                FOR j = 1 TO maxline - 1
                    IF maxdraw = maxlines THEN
                        pattern1$ = "## "
                        pattern2$ = "## "
                        LPRINT USING pattern1$; chart(i, maxnumbers);
                    END IF
                    PRINT USING "##"; chart(i, maxnumbers)
                NEXT j
            NEXT i
...
PRINT "How many Numbers [Maximum "; maxnum; " ] : ;: INPUT maxnum"
IF maxnum > maxdraw THEN PRINT "Too Many Numbers !": GOTO zzzzza
FOR zqa = 1 TO maxdraw
    rd2:
    IF saved$ = "Y" THEN
        GOTO szc
    END IF
    PRINT "Entry #"; zqa; " ";: INPUT ":", entry(zqa)
    IF entry(zqa) < 1 OR entry(zqa) > maxnumbers THEN PRINT "Number Out of Range ! , Try Again !!": GOTO rd2
    IF zqa > 1 THEN
        FOR i = 1 TO (zqa - 1)
...
FOR i = 1 TO linecounter
    ...
    bypass20:
    FOR j = 4 TO maxnumbers + bonus + bonus
        IF chart(i, j) = entrymax(maxnum) THEN
            match = match + 1
        END IF
        IF chart(i, j) = entrymax(bonus) THEN
            match = match + 1
        END IF
    NEXT j
NEXT i
...
IF match > 2 THEN
    PRINT "Match for : ";
    PRINT USING "\     \ ## ## #### "; month$(chart(i, 1)); chart(i, 2); vhart(i, 3);
    FOR i = 1 TO maxnumbers + bonus
        FOR j = 4 TO maxline - 1
            IF maxdraw = maxlines THEN
                pattern1$ = "## "
                pattern2$ = "## "
                PRINT USING pattern1$; chart(i, maxnumbers);
    ...
    IF hardcopy$ = "Y" THEN
        LPRINT "Match for : ";
        LPRINT USING "\    \ ## ## #### :'"; month$(chart(i, 1)); chart(i, 2); chart(i, 3);
        FOR i = 1 TO maxnumbers + bonus
            FOR j = 1 TO maxline - 1
                IF maxdraw = maxlines THEN
                    pattern1$ = "## "
...
PRINT
FOR zqa = 1 TO maxnumbers + bonus
    rd1:
    IF zqa = entry(zqa + bonus) AND bonus = 1 THEN
        PRINT "Bonus #"; "  "; " ";: INPUT entry(zqa)
    ELSE
        PRINT "Entry #"; zqa; " ";: INPUT entry(zqa)
    END IF
    IF entry(zqa) = 0 THEN GOTO searchmenu
    IF entry(zqa) < 1 OR entry(zqa) > maxnumbers THEN PRINT "Number Out of Range ! , Try Again !!": GOTO rd1
    IF zqa > 1 THEN
        FOR i = 1 TO (zqa - 1)
...
enterwinningnumbers:
OPEN lottodaata$ FOR APPEND AS #1
beginning:
CLS
PRINT
...
enternumbers:
FOR i = 1 TO maxdraw
    PRINT "Enter Number : ";: INPUT enternumber
    enternumber = enetrnumbers(i, maxdraw)
NEXT i
...
linecounter = 0
ERASE chart
OPEN lottodaata$ FOR INPUT AS #1
DO UNTIL EOF(1)
    linecounter = linecounter + 1
    INPUT #1, enternumbers(iinecounter, 1)
    INPUT #1, enternumbers(linecounter, 2)
    INPUT #1, enternumbers(linecounter, 3)
    FOR i = 4 TO maxdraw + bonus
        INPUT #1, enternumbers(linecounter, i)
        chart(linecounter, 1) = month
        chart(linecounter, 2) = day
        chart(linecounter, 3) = year
        chart(linecounter, i) = enternumbers(linecounter, i)
...
LOCATE 21, 12
PRINT "0000000001111111111222222222233333333334444444444555555555666666666670"
LOCATE 22, 12
PRINT "1234567890123456789012345678901234567890123456789012345678901234567890"
xyxty = 4 * 2
...
PRINT "for real prizes by matching 7 out of maxnos numbers."
PRINT
...
PRINT "wheeled system sets of 7 numbers from 1 to 70."
PRINT
...
PRINT "all 7 numbers from 1 to 70. "
PRINT
...
PRINT "Cash for 5/7  Numbers Matched                   = $96.maxnos"
